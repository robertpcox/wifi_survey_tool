...
import { renderAllRunsSection } from "./all-runs.mjs";
...
export function renderComparisonView({ entries = [], comparison = null, allRuns = null }) {
  const choices = entries.length
    ...
    : `<p>No same-route sibling runs — compare across all campus runs below.</p>`;
  return `
    ...
    ${comparison ? comparisonTable(comparison) : (entries.length ? `
      <p class="empty-state">Choose another completed result to compare with this run.</p>` : "")}
    ${renderAllRunsSection(allRuns)}`;
}
