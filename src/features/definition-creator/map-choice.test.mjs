...
test("map choice contrasts exact-click and POI-centre coordinates", () => {
  ...
  assert.equal(state.nodes.get("[data-poi-summary]").hidden, false);
  assert.equal(state.nodes.get("[data-map-choice-poi-action]").disabled, false);
  assert.equal(state.nodes.get("[data-map-choice-poi-action]").hidden, false);
  closeCreatorMapChoice(state.find);
  assert.equal(state.nodes.get("[data-map-choice]").hidden, true);
});

test("map choice hides an unavailable POI centre without no-POI copy", () => {
  const state = harness();
  showCreatorMapChoice(state.find, {
    clicked: { lng: 1, lat: 2, z: 3 },
    context: { poi: { id: "42", center: null } },
  });
  assert.equal(state.nodes.get("[data-poi-summary]").textContent, "");
  assert.equal(state.nodes.get("[data-poi-summary]").hidden, true);
  assert.equal(state.nodes.get("[data-map-choice-poi-action]").disabled, true);
  assert.equal(state.nodes.get("[data-map-choice-poi-action]").hidden, true);
