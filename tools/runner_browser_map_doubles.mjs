// FEATURE:      Runner browser MazeMap doubles
// SURFACE:      installBrowserDoubles()
// WHY TOGETHER: Map, layer, 3D, point-query, routing, and download stubs share one page runtime.
// STATE:        Browser globals record observable Runner behavior
// RULES:        Dynamic routing resolves only when the smoke explicitly releases it.
// PROVENANCE:   Runner planned and Dynamic room browser acceptance
export function installBrowserDoubles() {
    ...
    Data: {
      ...
      async getPoiAt({ lng, lat }, z) {
        return {
          ...
          properties: { buildingId: "1013513", buildingName: "Clinical Services Building",
            floorId: `floor-${z}`, floorName: `Level ${z}`, name: `Room ${z}`,
            poiId: `room-${z}`, zLevel: z },
        };
      },
      getRouteJSON(from, to) {
        return new Promise(resolve => { window.__resolveRunnerRoute = () => resolve({
          type: "FeatureCollection", features: [{ type: "Feature",
            properties: { z: from.zLevel }, geometry: { type: "LineString",
              coordinates: [[from.lngLat.lng, from.lngLat.lat], [to.lngLat.lng, to.lngLat.lat]] } }] }); });
      },
    },
  };
