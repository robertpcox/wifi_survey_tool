...
// RULES:        Route lines sit below buildings; guidance stays above; every launch gets one floor bar.
...
export function runnerMapStackFindings(state, expectedLaunches = 1) {
  ...
  for (const id of ["route-lines-lyr", "route-active-lyr"]) {
    if (state.layerPlacements[id] !== "mm-area-extrusion") {
      findings.push(`${id} is not below the area extrusion`);
    }
  }
  for (const id of ["wp-pts-lyr", "stop-pts-lyr"]) {
    if (state.layerPlacements[id] !== null) {
      findings.push(`${id} did not remain above the area extrusion`);
    }
  }
  return findings;
}
